...
public class DrawLine : MonoBehaviour
{
    ...
    {
        Line,
        Bresenham,  // Bresenham Soluthin1
        Bresenham2, // Bresenham Soluthin2
        Bresenham3, // Bresenham Soluthin3
        XiaolinWu,
    }
    ...
    {
        // let x1 < x2 && 0 < abs(dy) < dx
        bool isSteep = Mathf.Abs(y2 - y1) > Mathf.Abs(x2 - x1);
        if (isSteep)
        {
            MyCommon.Swap(ref x1, ref y1);
            MyCommon.Swap(ref x2, ref y2);
        }

        if (x1 > x2)
        {
            MyCommon.Swap(ref x1, ref x2);
            MyCommon.Swap(ref y1, ref y2);
        }
    ...
    static void DrawBresenhamLine2(Texture2D tex, int x1, int y1, int x2, int y2, Color c)
    {
        ...
        // slope_error = 2*▲x * DV(k) = ((k ∗ 2∗▲y + 2∗▲y) mod (2*▲x)) − (▲x)
        if (dx > dy)
        {
    ...
    static void DrawBresenhamLine3(Texture2D tex, int x1, int y1, int x2, int y2, Color c)
    {
        /*
			...
			And f(x, y) = 0 is Linear equation

            let e = f(x,y) = (y-y1)dx - (x-x1)dy
            e_x, e_y, e_xy: the error of dir
                ...
                e_y   =  (y-y1)dx - (x+1-x1)dy
                      =  e - dy
                      =  e_xy - dx
            // At first e = 0, so just draw (x1, y1)
    ...
    static void DrawXiaolinWuLine(Texture2D tex, int x1, int y1, int x2, int y2, Color c)
    {
        ...
        for (int x = x1, y = y1; x <= x2; x ++)
        {
            ...
            tex.SetPixel(isSteep ? y : x, isSteep ? x : y, c * brightness); // the main difference with Bresenham line
            slope_error += dy;
            if (slope_error >= 0)
            {
                y += ystep;
